    ...
    getClima(latitud, longitud)
      .then((response) => {
        ...
        setClima({
          ...
          imagen: `http://openweathermap.org/img/wn/${response.data.weather[0].icon}@2x.png`,
          velocidadViento: "viento " + response.data.wind.speed + " m/s",
          temperaturaFarenheit: response.data.main.temp + " °F",
          temperaturaCentrigrados:
            Number((response.data.main.temp - 32) / 1.8).toFixed(2) + " °C",
          ...
          <div>
            <div className="containerFilas">
              ...
              <p onClick={cambiarTemperatura}>
                {temperatura
                  ? `${clima.temperaturaFarenheit}`
                  : `${clima.temperaturaCentrigrados}`}
              </p>
            </div>
